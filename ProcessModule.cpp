...
void ProcessModule::createProcesses(vector<UserAccounts> accounts, int processCount, void *sharedMemory, Monitor& monitor) { //add monitory
      pid_t pid;
//pid_t pid = getpid();
   // kill(pid, SIGTERM);
    for (int k = 0; k < processCount; k++) {
