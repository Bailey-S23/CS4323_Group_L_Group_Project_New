...
TransferTo::TransferTo(vector<string> transactionDetails, void* sharedMemory) {
    ...
    int withdrawAccountBalance = withdrawAmount(withdrawAccount, amount);
    if (withdrawAccountBalance > -1)
    {
        int depositAccountBalance = depositAmount(depositAccount, amount);
        if (depositAccountBalance > -1)
        {
